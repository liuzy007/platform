...
public class TransactionContext implements XAResource {
	...
	public TransactionContext(final RemotingClient remotingClient,
			final String serverUrl, final TransactionSession session,
			final LongSequenceGenerator localTransactionIdGenerator,
			final int seconds) {
		super();
	...
	public boolean isInXATransaction() {
		return this.transactionId != null
				&& this.transactionId.isXATransaction();
	}

	public boolean isInLocalTransaction() {
		return this.transactionId != null
				&& this.transactionId.isLocalTransaction();
	}
	...
	private boolean equals(final Xid xid1, final Xid xid2) {
		...
		return xid1.getFormatId() == xid2.getFormatId()
				&& Arrays.equals(xid1.getBranchQualifier(),
						xid2.getBranchQualifier())
				&& Arrays.equals(xid1.getGlobalTransactionId(),
	...
	public void forget(final Xid xid) throws XAException {
		...
		final TransactionInfo info = new TransactionInfo(x, this.sessionId,
				TransactionInfo.TransactionType.FORGET);
		this.syncSendXATxCommand(info);
	}
	...
	public boolean isSameRM(final XAResource xaResource) throws XAException {
		...
		try {
			return this.getResourceManagerId().equals(
					xar.getResourceManagerId());
		} catch (final Throwable e) {
			throw (XAException) new XAException(
					"Could not get resource manager id.").initCause(e);
		}
	}
	...
	public Xid[] recover(final int flag) throws XAException {
		...
		try {
			...
			final BooleanCommand receipt = (BooleanCommand) this.remotingClient
					.invokeToGroup(this.serverUrl, new TransactionCommand(info,
	...
	private BooleanCommand syncSendXATxCommand(final TransactionInfo info)
			throws XAException {
		try {
			final BooleanCommand resp = (BooleanCommand) this.remotingClient
					.invokeToGroup(this.serverUrl, new TransactionCommand(info,
							OpaqueGenerator.getNextOpaque()));
	...
	private void throwRMFailException(final BooleanCommand resp)
			throws XAException {
	...
	private void endXATransaction() throws XAException {
		if (this.transactionId != null) {
			...
			final TransactionInfo info = new TransactionInfo(
					this.transactionId, this.sessionId,
					TransactionInfo.TransactionType.END);
			try {
				this.remotingClient.sendToGroup(
						this.serverUrl,
						new TransactionCommand(info, OpaqueGenerator
								.getNextOpaque()));
				if (LOG.isDebugEnabled()) {
	...
	private void startXATransaction(final Xid xid) throws XAException {
		...
		final TransactionInfo info = new TransactionInfo(this.transactionId,
				this.sessionId, TransactionInfo.TransactionType.BEGIN,
	...
	public void begin() throws MetaClientException {
		...
		if (this.transactionId == null) {
			...
			final TransactionInfo info = new TransactionInfo(
					this.transactionId, this.sessionId,
					TransactionInfo.TransactionType.BEGIN,
					this.transactionTimeout);
			try {
				this.checkConnectionConnected();
	...
	private MetaClientException toMetaClientException(final Exception e) {
		if (e instanceof MetaClientException) {
			...
			return new MetaClientException(e.getMessage(),
	...
	private void syncSendLocalTxCommand(final TransactionInfo info)
