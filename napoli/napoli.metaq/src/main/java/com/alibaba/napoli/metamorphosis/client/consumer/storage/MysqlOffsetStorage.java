...
public class MysqlOffsetStorage implements OffsetStorage {
		...
		JDBCUtils.execute(conn, new JDBCUtils.ConnectionCallback() {
			...
			public Object doInConnection(final Connection conn)
				...
				final String updateSQL = "update "
						+ MysqlOffsetStorage.this.tableName
						+ " set offset=?,msg_id=? where topic=? and  partition=?  and group_id=?";
				final PreparedStatement preparedStatement = conn
						.prepareStatement(updateSQL);
				JDBCUtils.execute(preparedStatement,
						new JDBCUtils.PreparedStatementCallback() {
							@Override
							public Object doInPreparedStatement(
								...
								for (final TopicPartitionRegInfo info : infoList) {
									...
									pstmt.setString(4, info.getPartition()
											.toString());
	...
	public TopicPartitionRegInfo load(final String topic, final String group,
		...
		final Connection conn = JDBCUtils.getConnection(this.dataSource);
		return (TopicPartitionRegInfo) JDBCUtils.execute(conn,
				new JDBCUtils.ConnectionCallback() {
					@Override
					public Object doInConnection(final Connection conn)
							throws SQLException {
						final String selectSQL = "select offset,msg_id from "
								+ MysqlOffsetStorage.this.tableName
								+ " where topic=? and partition=? and group_id=?";
						final PreparedStatement preparedStatement = conn
								.prepareStatement(selectSQL);
						return JDBCUtils.execute(preparedStatement,
								new JDBCUtils.PreparedStatementCallback() {
									@Override
									public Object doInPreparedStatement(
											final PreparedStatement pstmt)
										...
										pstmt.setString(2, partition.toString());
										pstmt.setString(3, group);
										final ResultSet rs = pstmt
												.executeQuery();
										return JDBCUtils
												.execute(
														rs,
														new JDBCUtils.ResultSetCallback() {

															@Override
															public Object doInResultSet(
																	final ResultSet rs)
																	throws SQLException {
																if (rs.next()) {
																	final long offset = rs
																			.getLong(1);
																	final long msgId = rs
																			.getLong(2);
																	return new TopicPartitionRegInfo(
																			topic,
																			partition,
																			offset,
																			msgId);
