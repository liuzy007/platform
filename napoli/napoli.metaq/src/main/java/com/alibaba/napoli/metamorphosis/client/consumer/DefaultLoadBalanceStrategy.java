...
public class DefaultLoadBalanceStrategy implements LoadBalanceStrategy {
	...
	public List<String> getPartitions(final String topic,
		...
		final int nPartsPerConsumer = curPartitions.size()
				/ curConsumers.size();
		// 挂载到额外partition的consumer数目
		final int nConsumersWithExtraPart = curPartitions.size()
				% curConsumers.size();
		...
		if (myConsumerPosition < 0) {
			log.warn("No broker partions consumed by consumer " + consumerId
					+ " for topic " + topic);
		...
		final int nParts = nPartsPerConsumer
				+ (myConsumerPosition + 1 > nConsumersWithExtraPart ? 0 : 1);
