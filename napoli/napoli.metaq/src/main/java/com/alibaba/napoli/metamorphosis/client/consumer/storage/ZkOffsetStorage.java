...
public class ZkOffsetStorage implements OffsetStorage, ZkClientChangedListener {
	...
	public ZkOffsetStorage(final MetaZookeeper metaZookeeper,
	...
	public void commitOffset(final String group,
		...
		for (final TopicPartitionRegInfo info : infoList) {
			...
			final ZKGroupTopicDirs topicDirs = this.metaZookeeper.new ZKGroupTopicDirs(
					topic, group);
			long newOffset = -1;
			long msgId = -1;
			// 加锁，保证msgId和offset一致
			...
			try {
				...
				if (!ZkUtils.pathExists(this.zkClient,
						topicDirs.consumerOffsetDir + "/"
								+ info.getPartition().toString())) {
					Partition partition = info.getPartition();
					int brokerId = partition.getBrokerId();
					String brokerTopicPath = this.metaZookeeper
							.brokerTopicsPathOf(topic, brokerId, -1);
				...
				ZkUtils.updatePersistentPath(this.zkClient,
						topicDirs.consumerOffsetDir + "/"
								+ info.getPartition().toString(), msgId + "-"
								+ newOffset);
			} catch (final Throwable t) {
				log.error("exception during commitOffsets", t);
			}
			if (log.isDebugEnabled()) {
				log.debug("Committed offset " + newOffset + " for topic "
						+ info.getTopic());
			}
	...
	public TopicPartitionRegInfo load(final String topic, final String group,
		...
		final String znode = topicDirs.consumerOffsetDir + "/"
				+ partition.toString();
		final String offsetString = ZkUtils.readDataMaybeNull(this.zkClient,
				znode);
		if (offsetString == null) {
			...
			if (index > 0) {
				// 1.4开始的新客户端
				final long msgId = Long.parseLong(offsetString.substring(0,
						index));
