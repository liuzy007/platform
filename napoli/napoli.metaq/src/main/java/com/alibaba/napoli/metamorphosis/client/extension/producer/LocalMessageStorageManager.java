...
import com.alibaba.napoli.common.store.Store;
import com.alibaba.napoli.common.store.journal.JournalStore;
import com.alibaba.napoli.metamorphosis.Message;
import com.alibaba.napoli.metamorphosis.cluster.Partition;
import com.alibaba.napoli.metamorphosis.exception.GetRecoverStorageErrorException;
import com.alibaba.napoli.metamorphosis.exception.UnknowCodecTypeException;
import com.alibaba.napoli.metamorphosis.utils.IdWorker;
...
public class LocalMessageStorageManager implements MessageRecoverManager {
	 ...
	 */
	protected final ConcurrentHashMap<String/* topic@partition */, FutureTask<Store>> topicStoreMap = new ConcurrentHashMap<String, FutureTask<Store>>();

	/**
	 * 表示正在执行的恢复任务的map
	 */
	protected final ConcurrentHashMap<String/* topic@partition */, FutureTask<Boolean>> topicRecoverTaskMap = new ConcurrentHashMap<String, FutureTask<Boolean>>();
	...
	public static final String DEFAULT_META_LOCALMESSAGE_PATH = System
			.getProperty("meta.localmessage.path",
					System.getProperty("user.home") + File.separator
							+ ".meta_localmessage");

	public String META_LOCALMESSAGE_PATH;

	private final String META_LOCALMESSAGE_CODEC_TYPE = System.getProperty(
			"meta.localmessage.codec", "java");
	...
	public LocalMessageStorageManager(final MetaClientConfig metaClientConfig,
			final String path, final MessageRecoverer messageRecoverer) {
		super();
		this.META_LOCALMESSAGE_PATH = StringUtils.isNotBlank(path) ? path
		...
		if (this.META_LOCALMESSAGE_CODEC_TYPE.equals("java")) {
			...
			throw new UnknowCodecTypeException(
					this.META_LOCALMESSAGE_CODEC_TYPE);
		}

		// 跟接收消息的RecoverThreadCount一样
		this.threadPoolExecutor = new ThreadPoolExecutor(
				metaClientConfig.getRecoverThreadCount(),
		...
		}, 0, metaClientConfig.getRecoverMessageIntervalInMills(),
	 ...
	 * */
	@Override
	public boolean recover(final String topic, final Partition partition,
			final MessageRecoverer recoverer) {

		final String name = this.generateKey(topic, partition);
		final FutureTask<Boolean> recoverTask = new FutureTask<Boolean>(
				new Callable<Boolean>() {
					...
					public Boolean call() throws Exception {
						...
						try {
							...
							final Store store = LocalMessageStorageManager.this
									.getOrCreateStore(topic, partition);

							this.innerRecover(store, recoverer, count, name);
						} catch (final Throwable e) {
							log.error("SendRecover发送消息恢复失败,name=" + name, e);
						} finally {
							log.info("SendRecover执行完毕移除发送恢复任务,name=" + name
									+ ",恢复消息" + count.get() + "条");
							LocalMessageStorageManager.this.topicRecoverTaskMap
									.remove(name);
						}
						return true;
					}

					private void innerRecover(final Store store,
							final MessageRecoverer recoverer,
							final AtomicLong count, final String name)
						...
						while (it.hasNext()) {
							...
							try {
								...
								if (count.get() % 20000 == 0) {
									log.info("SendRecover " + name + "已恢复消息条数:"
											+ count.get());
								}
							} catch (final IOException e) {
								log.error("SendRecover remove message failed",
										e);
							}
						}
					}
		...
		final FutureTask<Boolean> ret = this.topicRecoverTaskMap.putIfAbsent(
				name, recoverTask);
		if (ret == null) {
	...
	protected Store getOrCreateStore(final String topic,
	...
	private Store getOrCreateStore0(final String topic,
			final Partition partition) {
		final String name = this.generateKey(topic, partition);
		...
		if (task != null) {
			...
			task = new FutureTask<Store>(new Callable<Store>() {
				...
				public Store call() throws Exception {
					final File file = new File(
							LocalMessageStorageManager.this.META_LOCALMESSAGE_PATH
									+ File.separator + name);
					if (!file.exists()) {
						file.mkdir();
					}
			...
			FutureTask<Store> existsTask = this.topicStoreMap.putIfAbsent(name,
					task);
			if (existsTask == null) {
				task.run();
				existsTask = task;
	...
	private Store getStore(final String topic, final FutureTask<Store> task) {
		try {
			...
			throw new GetRecoverStorageErrorException("获取topic=" + topic
					+ "对应的store失败", t);
		}
	...
	public void shutdown() {
		for (final Map.Entry<String, FutureTask<Store>> entry : this.topicStoreMap
				.entrySet()) {
	...
	@Override
	public void append(final Message message, final Partition partition)
			throws IOException {
		final Store store = this
				.getOrCreateStore(message.getTopic(), partition);
	...
	@Override
	synchronized public void setMessageRecoverer(
			final MessageRecoverer recoverer) {
		if (this.messageRecoverer == null) {
	...
	protected Store newStore(final String name) throws IOException {
		return new JournalStore(this.META_LOCALMESSAGE_PATH + File.separator
				+ name, name);
	}
