...
public class OrderedLocalMessageStorageManager extends
	...
	public void recover() {
		...
		if (this.messageRecoverer != null) {
			for (String name : names) {
				...
				if (!partition.equals(Partition.RandomPartiton)
						&& this.getMessageCount(topic, partition) > 0) {
	...
	public boolean recover(final String topic, final Partition partition,
		...
		FutureTask<Boolean> recoverTask = new FutureTask<Boolean>(
				new Callable<Boolean>() {
					...
					public Boolean call() throws Exception {
						try {
							int count = 0;
							Store randomPartitonStore = OrderedLocalMessageStorageManager.this
									.getOrCreateStore(topic,
							...
							if (randomPartitonStore.size() > 0) {
								...
								synchronized (randomPartitonStore) {
									// 双重检测
									if (randomPartitonStore.size() > 0) {
										count = this.innerRecover(
												randomPartitonStore, recoverer);
										log.info("SendRecover topic=" + topic
												+ "@-1--1恢复消息" + count + "条");
									}
								}
							}

							Store store = OrderedLocalMessageStorageManager.this
									.getOrCreateStore(topic, partition);

							count = this.innerRecover(store, recoverer);
							log.info("SendRecover topic=" + name + "恢复消息"
									+ count + "条");
						} catch (Throwable e) {
							log.error("SendRecover发送消息恢复失败,topic=" + name, e);
						} finally {
							log.info("SendRecover执行完毕移除发送恢复任务,topic=" + name);
							OrderedLocalMessageStorageManager.this.topicRecoverTaskMap
									.remove(name);
						}
						return true;
					}

					private int innerRecover(Store store,
							final MessageRecoverer recoverer)
						...
						while (it.hasNext()) {
							byte[] key = it.next();
							Message msg = (Message) OrderedLocalMessageStorageManager.this.deserializer
		...
		FutureTask<Boolean> ret = this.topicRecoverTaskMap.putIfAbsent(name,
				recoverTask);
		if (ret == null) {
	...
	protected Store newStore(final String name) throws IOException {
		return new MessageStore(META_LOCALMESSAGE_PATH + File.separator + name,
				name);
	}
