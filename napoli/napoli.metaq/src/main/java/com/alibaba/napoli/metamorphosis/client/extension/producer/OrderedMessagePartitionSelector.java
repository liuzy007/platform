...
// 留出扩展余地
public abstract class OrderedMessagePartitionSelector extends
	...
	public Partition getPartition(String topic, List<Partition> partitions,
		...
		if (configPartitionsNum == 0) {
			throw new MetaClientException(
					"There is no config partitions for topic " + topic
							+ ",maybe you don't config it at first?");
		}

		Partition selectedPartition = this.choosePartition(topic,
				configPartitions, message);
		...
		if (availablePartitionNum == 0) {
			throw new AvailablePartitionNumException("selected partition["
					+ selectedPartition + "]for topic[" + topic
					+ "]can not write now");
		}

		// 可用分区和配置分区均不包含选出来的分区,(可能是用户乱填的,抛出异常用户自己负责)
		if (!configPartitions.contains(selectedPartition)
				&& !partitions.contains(selectedPartition)) {
			throw new MetaClientException("invalid selected partition:"
					+ selectedPartition
	...
	protected abstract Partition choosePartition(String topic,
			List<Partition> partitions, Message message);
