...
public class ProducerZooKeeper implements ZkClientChangedListener {
	...
	final class BrokerConnectionListener implements IZkChildListener,IZkDataListener{
		...
		volatile BrokersInfo brokersInfo = new BrokersInfo(
		...
		private Map<String,List<Partition>> filterPartitionMap(Map<String, List<Partition>> oldPartitionsPerTopicMap){
		...
		void syncedUpdateBrokersInfo() throws NotifyRemotingException,
			...
			try {
				...
				log.warn("Begin receiving broker changes for topic "
						+ this.topic + ",broker ids:" + newTopicPartitionMap);

				for (final Map.Entry<Integer, String> oldEntry : this.brokersInfo.oldBrokerStringMap
						.entrySet()) {
					final Integer oldBrokerId = oldEntry.getKey();
					final String oldBrokerString = oldEntry.getValue();
					final String newBrokerString = newBrokerStringMap
							.get(oldBrokerId);
					// 新旧都有
					if (newBrokerStringMap.containsKey(oldBrokerId)) {
						// 判断内容是否变化
						if (!newBrokerString.equals(oldBrokerString)) {
							ProducerZooKeeper.this.remotingClient.close(
									oldBrokerString, false);
							ProducerZooKeeper.this.remotingClient
									.connect(newBrokerString);
							ProducerZooKeeper.this.remotingClient
									.awaitReadyInterrupt(newBrokerString);
							log.warn("Close " + oldBrokerString
									+ ",connect to " + newBrokerString);
						} else {
							// ignore
						}
					} else {
				...
				for (final Map.Entry<Integer, String> newEntry : newBrokerStringMap
						.entrySet()) {
					final Integer newBrokerId = newEntry.getKey();
					final String newBrokerString = newEntry.getValue();
					// 新的有，旧的没有，创建
					if (!this.brokersInfo.oldBrokerStringMap
							.containsKey(newBrokerId)) {
						ProducerZooKeeper.this.remotingClient
								.connect(newBrokerString);
						ProducerZooKeeper.this.remotingClient
								.awaitReadyInterrupt(newBrokerString);
				...
				this.brokersInfo = new BrokersInfo(newBrokerStringMap,
	...
	public ProducerZooKeeper(final MetaZookeeper metaZookeeper,
	...
	public void publishTopic(final String topic) {
		...
		final FutureTask<BrokerConnectionListener> task = new FutureTask<BrokerConnectionListener>(
				new Callable<BrokerConnectionListener>() {
					...
					public BrokerConnectionListener call() throws Exception {
						final BrokerConnectionListener listener = new BrokerConnectionListener(
								topic);
						if (ProducerZooKeeper.this.zkClient != null) {
							ProducerZooKeeper.this.publishTopicInternal(topic,
									listener);
						}
						return listener;
					}
		...
		final FutureTask<BrokerConnectionListener> existsTask = this.topicConnectionListeners
				.putIfAbsent(topic, task);
		if (existsTask == null) {
			task.run();
		}
	}

	private void publishTopicInternal(final String topic,
			final BrokerConnectionListener listener) throws Exception,
			NotifyRemotingException, InterruptedException {
		final String partitionPath = this.metaZookeeper.brokerTopicsPath + "/"
				+ topic;
		ZkUtils.makeSurePersistentPathExists(ProducerZooKeeper.this.zkClient,
				partitionPath);
		ProducerZooKeeper.this.zkClient.subscribeChildChanges(partitionPath,
				listener);
	...
	BrokerConnectionListener getBrokerConnectionListener(final String topic) {
		final FutureTask<BrokerConnectionListener> task = this.topicConnectionListeners
				.get(topic);
		if (task != null) {
			try {
				return task.get();
	...
	Set<String> getServerUrlSetByTopic(final String topic) {
		final BrokerConnectionListener brokerConnectionListener = this
				.getBrokerConnectionListener(topic);
	...
	public synchronized void setDefaultTopic(final String topic) {
		if (this.defaultTopic != null && !this.defaultTopic.equals(topic)) {
			throw new IllegalStateException(
					"Default topic has been setup already:" + this.defaultTopic);
		}
	 ...
	 * @return
	 */
	Partition selectPartition(final String topic, final Message msg,
			final PartitionSelector selector, final String serverUrl)
			throws MetaClientException {
		final BrokerConnectionListener brokerConnectionListener = this
				.getBrokerConnectionListener(topic);
		if (brokerConnectionListener != null) {
			final BrokersInfo brokersInfo = brokerConnectionListener.brokersInfo;
			final List<Partition> partitions = brokersInfo.oldTopicPartitionMap
					.get(topic);
			...
			for (final Partition partition : partitions) {
				if (serverUrl.equals(brokerStringMap.get(partition
						.getBrokerId()))) {
	...
	public String selectBroker(final String topic, final Partition partition) {
		...
		if (partition != null) {
			...
			if (brokerConnectionListener != null) {
				final BrokersInfo brokersInfo = brokerConnectionListener.brokersInfo;
				return brokersInfo.oldBrokerStringMap.get(partition
						.getBrokerId());
			} else {
	 ...
	 * @return
	 */
	private String selectDefaultBroker(final String topic,
	...
	public Partition selectPartition(final String topic, final Message message,
		...
		if (brokerConnectionListener != null) {
			final BrokersInfo brokersInfo = brokerConnectionListener.brokersInfo;
			return partitionSelector.getPartition(topic,
					brokersInfo.oldTopicPartitionMap.get(topic), message);
		} else {
			return this.selectDefaultPartition(topic, message,
					partitionSelector, null);
		}
	}

	private Partition selectDefaultPartition(final String topic,
			final Message message, final PartitionSelector partitionSelector,
		...
		if (brokerConnectionListener != null) {
			...
			if (serverUrl == null) {
				return partitionSelector
						.getPartition(this.defaultTopic,
								brokersInfo.oldTopicPartitionMap
										.get(this.defaultTopic), message);
			} else {
				final List<Partition> partitions = brokersInfo.oldTopicPartitionMap
						.get(this.defaultTopic);
				final Map<Integer/* broker id */, String/* server url */> brokerStringMap = brokersInfo.oldBrokerStringMap;
				// 过滤特定broker的分区列表
				final List<Partition> partitionsForSelect = new ArrayList<Partition>();
				for (final Partition partition : partitions) {
	...
	public void onZkClientChanged(final ZkClient newClient) {
		...
		try {
			for (final String topic : this.topicConnectionListeners.keySet()) {
				...
				this.publishTopicInternal(topic,
						this.getBrokerConnectionListener(topic));
			}
