...
class OrderedMessageSender {
	...
	SendResult sendMessage(Message message, long timeout, TimeUnit unit)
		...
		for (;;) {
			...
			if (!result.isPartitionWritable()) {
				return this.producer.saveMessageToLocal(message,
						result.getSelectedPartition(), timeout, unit);
			}
			// 可用分区正常
			else {
				int localMessageCount = this.producer.getLocalMessageCount(
						message.getTopic(), result.getSelectedPartition());
				if (localMessageCount > 0) {
					this.producer.tryRecoverMessage(message.getTopic(),
							result.getSelectedPartition());
				}

				if (localMessageCount <= 0) {
					// 本地存的消息条数为空,本次消息发送到服务端
					return this.producer.sendMessageToServer(message, timeout,
							unit, true);
	...
	private SelectPartitionResult trySelectPartition(Message message)
		...
		try {
			...
			String partitionStr = msg.substring(msg.indexOf("[") + 1,
					msg.indexOf("]"));
