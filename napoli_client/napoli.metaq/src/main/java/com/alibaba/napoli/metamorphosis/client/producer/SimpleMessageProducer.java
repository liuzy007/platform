...
import com.alibaba.napoli.metamorphosis.client.transaction.TransactionContext;
import com.alibaba.napoli.metamorphosis.client.transaction.TransactionSession;
...
import com.alibaba.napoli.metamorphosis.network.ByteUtils;
...
import com.alibaba.napoli.metamorphosis.utils.MessageFlagUtils;
...
public class SimpleMessageProducer implements MessageProducer,
	...
	private static final Log log = LogFactory
			.getLog(SimpleMessageProducer.class);
	protected static final long DEFAULT_OP_TIMEOUT = 3000L;
	private static final int TIMEOUT_THRESHOLD = Integer.parseInt(System
			.getProperty("meta.send.timeout.threshold", "200"));
	...
	@Override
	public SendResult sendMessage(final Message message, final long timeout,
	...
	/** 正常的消息发送到服务器 */
	protected SendResult sendMessageToServer(final Message message,
		...
		int retry = 0;
		final long timeoutInMills = TimeUnit.MILLISECONDS
		...
		try {
			for (int i = 0; i < MAX_RETRY; i++) {
				...
				if (System.currentTimeMillis() - start >= timeoutInMills) {
					throw new MetaOpeartionTimeoutException(
							"Send message timeout in " + timeoutInMills
									+ " mills");
				}
				retry++;
			}
			...
			MetaStatLog.addStatValue2(null, StatConstants.PUT_TIME_STAT,
					message.getTopic(), duration);
			if (duration > TIMEOUT_THRESHOLD) {
				MetaStatLog.addStatValue2(null, StatConstants.PUT_TIMEOUT_STAT,
						message.getTopic(), duration);
			}
			if (retry > 0) {
				MetaStatLog.addStatValue2(null, StatConstants.PUT_RETRY_STAT,
						message.getTopic(), retry);
			}
		}
		return result;
	}
	...
	private Partition selectPartition(final Message message)
		...
		return this.producerZooKeeper.selectPartition(message.getTopic(),
				message, this.partitionSelector);
	}
	...
	public void setTransactionTimeout(final int seconds)
		...
		if (seconds < 0) {
			throw new IllegalArgumentException(
					"Illegal transaction timeout value");
		}
	...
	public void beginTransaction() throws MetaClientException {
		...
		if (ctx == null) {
			ctx = new TransactionContext(this.remotingClient, null, this,
					this.localTxIdGenerator, this.transactionTimeout);
	 ...
	 * @throws MetaClientException
	 */
	protected void beforeSendMessageFirstTime(final String serverUrl)
			throws MetaClientException, XAException {
	...
	private SendResult send0(final Message message, final byte[] encodedData,
			final long timeout, final TimeUnit unit)
		...
		try {
			...
			if (this.isInTransaction()) {
				...
				if (info != null) {
					...
					// 选择该broker内的某个分区
					partition = this.producerZooKeeper.selectPartition(topic,
							message, this.partitionSelector, serverUrl);
					if (partition == null) {
						// 没有可用分区，抛出异常
						throw new MetaClientException(
								"There is no partitions in `" + serverUrl
										+ "` to send message with topic `"
										+ topic + "` in a transaction");
					}
				}
			}
			if (partition == null) {
			...
			if (serverUrl == null) {
				serverUrl = this.producerZooKeeper.selectBroker(topic,
						partition);
			}
			if (serverUrl == null) {
				throw new MetaClientException(
						"There is no aviable server right now for topic "
			...
			final PutCommand putCommand = new PutCommand(topic,
					partition.getPartition(), encodedData,
					this.getTransactionId(), flag,
					OpaqueGenerator.getNextOpaque());
			final BooleanCommand resp = this.invokeToGroup(serverUrl,
					partition, putCommand, message, timeout, unit);
	...
	private SendResult genSendResult(final Message message,
			final Partition partition, final String serverUrl,
		...
		case HttpStatus.Success: {
			...
			MessageAccessor.setId(message, Long.parseLong(tmps[1]));
			final Partition serverPart = new Partition(
					Integer.parseInt(tmps[2]), Integer.parseInt(tmps[3]));
			MessageAccessor.setPartition(message, serverPart);
			// 记录本次发送信息，仅用于事务消息
			this.logLastSentInfo(serverUrl);
			return new SendResult(true, serverPart, Long.parseLong(tmps[4]),
					resultStr);
		}
		case HttpStatus.Forbidden: {
			...
			return new SendResult(false, null, -1,
	...
	protected BooleanCommand invokeToGroup(final String serverUrl,
			final Partition partition, final PutCommand putCommand,
			final Message message, final long timeout, final TimeUnit unit)
			throws InterruptedException, TimeoutException,
			NotifyRemotingException {

		return (BooleanCommand) this.remotingClient.invokeToGroup(serverUrl,
				putCommand, timeout, unit);
	}
	...
	public static byte[] encodeData(final Message message) {
		...
		final ByteBuffer buffer = ByteBuffer.allocate(4 + attrLen
				+ payload.length);
		if (attribute != null) {
	...
	public void sendMessage(final Message message,
			final SendMessageCallback cb, final long time, final TimeUnit unit) {
		try {
			final String topic = message.getTopic();
			...
			final String serverUrl = this.producerZooKeeper.selectBroker(topic,
					partition);
			if (serverUrl == null) {
				throw new MetaClientException(
						"There is no aviable server right now for topic "
			...
			final int flag = MessageFlagUtils.getFlag(message);
			final byte[] encodedData = SimpleMessageProducer
					.encodeData(message);
			final PutCommand putCommand = new PutCommand(topic,
					partition.getPartition(), encodedData,
					this.getTransactionId(), flag,
					OpaqueGenerator.getNextOpaque());
			this.remotingClient.sendToGroup(serverUrl, putCommand,
					new SingleRequestCallBackListener() {
						@Override
						public void onResponse(
								final ResponseCommand responseCommand,
								final Connection conn) {
							final SendResult rt = SimpleMessageProducer.this
									.genSendResult(message, partition,
											serverUrl,
	...
	@Override
	public SendResult sendMessage(final Message message)
			throws MetaClientException, InterruptedException {
		return this.sendMessage(message, DEFAULT_OP_TIMEOUT,
	...
	@Override
	public boolean makeUrgentMessage(String messageContent) {
		if (StringUtils.isBlank(messageContent)) {
			return false;
		}
		try {
			String[] messageArray = StringUtils.split(messageContent);
			String messageId = messageArray[1];
			MetaZookeeperHelper helper = new MetaZookeeperHelper(
					producerZooKeeper.getMetaZookeeper());
			helper.addUrgentMessage(messageId, messageContent);
			return true;
		} catch (Exception e) {
