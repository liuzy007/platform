...
import com.alibaba.napoli.metamorphosis.client.extension.storage.MessageStore;
...
public class RecoverStorageManager extends AbstractRecoverManager {
	...
	private final ConcurrentHashMap<String/* topic */, FutureTask<Store>> topicStoreMap = new ConcurrentHashMap<String, FutureTask<Store>>();
	...
	public static final String META_RECOVER_STORE_PATH = System.getProperty(
			"meta.recover.path", System.getProperty("user.home")
					+ File.separator + ".meta_recover");
	...
	public synchronized void start(final MetaClientConfig metaClientConfig) {
		...
		}, 5000, metaClientConfig.getRecoverMessageIntervalInMills(),
		...
		for (final Map.Entry<String, FutureTask<Store>> entry : this.topicStoreMap
			...
			this.threadPoolExecutor.execute(new Runnable() {
				...
				public void run() {
					...
					final Store store = RecoverStorageManager.this.getStore(
							topic, task);
					try {
						final MessageListener listener = RecoverStorageManager.this.subscribeInfoManager
								.getMessageListener(topic, group);
						final Iterator<byte[]> it = store.iterator();
						int count = 0;
						while (it.hasNext()) {
							final byte[] key = it.next();
							final Message msg = (Message) RecoverStorageManager.this.deserializer
									.decodeObject(store.get(key));
							if (msg != null) {
								RecoverStorageManager.this.receiveMessage(
										store, key, msg, listener);
							}
							count++;
						}
						log.info("Recover topic=" + topic + "恢复消息" + count
								+ "条");
	...
	private void receiveMessage(final Store store, final byte[] key,
			final Message msg, final MessageListener messageListener) {
		...
		if (messageListener.getExecutor() != null) {
			try {
				messageListener.getExecutor().execute(new Runnable() {
					...
					@Override
					public void run() {
						RecoverStorageManager.this.notifyListener(store, key,
								msg, messageListener);
					}
	...
	private void notifyListener(final Store store, final byte[] key,
			final Message msg, final MessageListener messageListener) {
	...
	@Override
	public void append(final String group, final Message message)
			throws IOException {
		...
		for (int i = 0; i < 5; i++) {
			try {
				...
				if (msg.contains("重复")) {
					error = e;
					log.warn("写入recover store出错,key=" + key + ","
							+ e.getMessage() + ",retry...");
		...
		if (task != null) {
			...
			task = new FutureTask<Store>(new Callable<Store>() {
				...
				public Store call() throws Exception {
					...
					return new MessageStore(META_RECOVER_STORE_PATH
							+ File.separator + name, name);
				}
	...
	private Store getStore(final String name, final FutureTask<Store> task) {
		try {
			...
			throw new GetRecoverStorageErrorException("获取topic=" + name
					+ "对应的store失败", t);
		}
