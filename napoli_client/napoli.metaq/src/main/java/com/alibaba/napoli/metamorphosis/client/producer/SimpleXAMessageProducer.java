...
import com.alibaba.napoli.gecko.core.util.ConcurrentHashSet;
import com.alibaba.napoli.metamorphosis.exception.InvalidBrokerException;
 ...
 * 
 */
public class SimpleXAMessageProducer extends SimpleMessageProducer implements
		XAMessageProducer {

	public SimpleXAMessageProducer(
	...
	private String selectTransactionBroker() {
		...
		for (final String topic : this.publishedTopics) {
			brokerUrls
					.add(this.producerZooKeeper.getServerUrlSetByTopic(topic));
		}
		final Set<String> resultSet = intersect(brokerUrls);
		if (resultSet.isEmpty()) {
			throw new InvalidBrokerException(
					"Could not select a common broker url for  topics:"
