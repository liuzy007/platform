...
public class ConsisHashStrategy implements LoadBalanceStrategy {
	...
	@Override
	public List<String> getPartitions(final String topic,
			final String consumerId, final List<String> curConsumers,
			final List<String> curPartitions) {

		final TreeMap<Long, String> consumerMap = this
				.buildConsumerMap(curConsumers);

		final Set<String> rt = new HashSet<String>();
		// 根据partition查找对应的consumer
		for (final String partition : curPartitions) {
			final String targetConsumer = this.findConsumerByPartition(
					consumerMap, partition);
			// 保存本consumer需要挂载的分区
	...
	private String findConsumerByPartition(
			final TreeMap<Long, String> consumerMap, final String partition) {
	...
	private TreeMap<Long, String> buildConsumerMap(
			final List<String> curConsumers) {
		...
		for (final String consumer : curConsumers) {
			if (this.alg == HashAlgorithm.KETAMA_HASH) {
				for (int i = 0; i < NUM_REPS / 4; i++) {
					final byte[] digest = HashAlgorithm.computeMd5(consumer
							+ "-" + i);
					for (int h = 0; h < 4; h++) {
