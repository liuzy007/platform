...
public class FetchRequest implements Delayed {
	...
	public void increaseMaxSize() {
		if (this.maxSize > MessageUtils.MAX_READ_BUFFER_SIZE) {
			log.warn("警告：maxSize超过最大限制"
					+ MessageUtils.MAX_READ_BUFFER_SIZE
	...
	public FetchRequest(final Broker broker, final long delay,
			final TopicPartitionRegInfo topicPartitionRegInfo, final int maxSize) {
		super();
		this.broker = broker;
		this.delay = delay;
		if (delay >= 0) {
	...
	public boolean equals(final Object obj) {
		...
		} else if (!this.topicPartitionRegInfo
				.equals(other.topicPartitionRegInfo)) {
			return false;
		}
		return true;
	}
	...
	public long getDelay(final TimeUnit unit) {
		return unit.convert(this.delayTimeStamp - System.currentTimeMillis(),
