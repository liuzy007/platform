...
public class AsyncMetaMessageProducer extends SimpleMessageProducer implements
		AsyncMessageProducer, MessageRecoverer {
	private static final Log log = LogFactory
			.getLog(AsyncMetaMessageProducer.class);
	...
	public AsyncMetaMessageProducer(
			...
			final int slidingWindowSize0, final IgnoreMessageProcessor processor) {

		super(messageSessionFactory, remotingClient, partitionSelector,
				producerZooKeeper, sessionId);
		this.slidingWindow = new SlidingWindow(
				slidingWindowSize0 > 0 ? slidingWindowSize0 : DEFAULT_PERMITS);
		this.ignoreMessageProcessor = processor != null ? processor
				: new AsyncIgnoreMessageProcessor(
	...
	public void asyncSendMessage(final Message message) {
		this.asyncSendMessage(message, DEFAULT_OP_TIMEOUT,
	 ...
	 */
	@Override
	public void asyncSendMessage(final Message message, final long timeout,
			final TimeUnit unit) {
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug("save to local strage,and waitting for recover. cause:"
						+ e.getMessage());
			}
			...
			if (log.isDebugEnabled()) {
				log.debug(
						"save to local strage,and waitting for recover. cause:",
						e);
			}
			this.handleSendFailMessage(message);
		}
	 ...
	 */
	@Override
	public void setIgnoreMessageProcessor(
			final IgnoreMessageProcessor ignoreMessageProcessor) {
	...
	private BooleanCommand trySend(final String serverUrl,
			final PutCommand putCommand, final long timeout, final TimeUnit unit)
			throws NotifyRemotingException, InterruptedException {
		final int dataLength = putCommand.getData() != null ? putCommand
				.getData().length : 0;
		if (this.slidingWindow.tryAcquireByLength(dataLength)) {// , timeout,
																// unit
			try {
				super.remotingClient.sendToGroup(serverUrl, putCommand,
						new MessageSendCallBackListener(putCommand), timeout,
						unit);
				return new BooleanCommand(putCommand.getOpaque(),
						HttpStatus.Success, "-1 " + putCommand.getPartition()
								+ " -1");
			} catch (final NotifyRemotingException e) {
				this.slidingWindow.releaseByLenth(dataLength);
				if (e.getMessage().contains("超过流量限制")
						|| e.getMessage().contains("超过允许的最大CallBack个数")) {
			...
			throw new MetaMessageOverflowException("发送消息流量超过滑动窗口单位总数："
					+ this.slidingWindow.getWindowsSize());
		}
	}

	private BooleanCommand processOverMessage(final Partition partition,
			final PutCommand putCommand, final Message message,
			final MetaMessageOverflowException e2)
			throws MetaMessageOverflowException {
	 ...
	 * 
	 */
	public static class MetaMessageOverflowException extends
			NotifyRemotingException {
	...
	private class MessageSendCallBackListener implements
			SingleRequestCallBackListener {
		...
		@Override
		public void onResponse(final ResponseCommand responseCommand,
				final Connection conn) {
			this.release();

			if (responseCommand.getResponseStatus() != ResponseStatus.NO_ERROR) {
				final StringBuilder sb = new StringBuilder();
				sb.append("onResponse. Status:").append(
						responseCommand.getResponseStatus());
				if (responseCommand instanceof BooleanCommand) {
					sb.append(",Code:").append(
							((BooleanCommand) responseCommand).getCode());
				}
				if (responseCommand instanceof BooleanAckCommand) {
					sb.append(",ErrorMsg:")
							.append(((BooleanAckCommand) responseCommand)
									.getErrorMsg());
					sb.append(",ResponseHost:").append(
							((BooleanAckCommand) responseCommand)
									.getResponseHost());
				}
		...
		private void release() {
			if (this.released.compareAndSet(false, true)) {
				AsyncMetaMessageProducer.this.slidingWindow
						.releaseByLenth(this.dataLenth);
			}
		}
