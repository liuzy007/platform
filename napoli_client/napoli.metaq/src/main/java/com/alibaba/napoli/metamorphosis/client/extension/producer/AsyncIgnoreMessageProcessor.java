...
class AsyncIgnoreMessageProcessor implements IgnoreMessageProcessor {
	...
	private static final Log log = LogFactory
			.getLog(AsyncIgnoreMessageProcessor.class);

	private static final String STORAGE_PATH = System.getProperty(
			"meta.async.storage.path", System.getProperty("user.home")
					+ File.separator + ".meta_async_storage");
	...
	AsyncIgnoreMessageProcessor(MetaClientConfig metaClientConfig,
			MessageRecoverer recoverer) {
		this.storageManager = new LocalMessageStorageManager(metaClientConfig,
				STORAGE_PATH, recoverer);
	}
	...
	public boolean handle(Message message) throws Exception {
		...
		int count = this.storageManager.getMessageCount(message.getTopic(),
				partition);
