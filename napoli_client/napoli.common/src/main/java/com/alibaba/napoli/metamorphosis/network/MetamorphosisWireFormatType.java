...
import com.alibaba.napoli.gecko.core.command.CommandFactory;
import com.alibaba.napoli.gecko.core.command.CommandHeader;
...
import com.alibaba.napoli.gecko.core.util.ByteBufferMatcher;
import com.alibaba.napoli.gecko.core.util.OpaqueGenerator;
import com.alibaba.napoli.gecko.core.util.ShiftAndByteBufferMatcher;
import com.alibaba.napoli.gecko.service.config.WireFormatType;
            ...
            return new Decoder() {
                ...
                public Object decode(final IoBuffer buff, final Session session) {
                    ...
                    if (index >= 0) {
                        ...
                        switch (op) {
                        case 'p':
                        	if (tmps[0].equals(MetaEncodeCommand.PRIORITIZE_CMD)) {
                                return this.decodePrioritizeMessage(buff, tmps);
                            }
                            else {
                ...
                private Object decodePrioritizeMessage(final IoBuffer buff,final String[] tmps){
                	String topic = tmps[1];
                	int brokerId = Integer.parseInt(tmps[2]);
                	int partition = Integer.parseInt(tmps[3]);
                	int valueLen = Integer.parseInt(tmps[4]);
                	byte[] data = new byte[valueLen];
                    buff.get(data);
                    return new PrioritizeMessageCommand(topic,brokerId,partition,data);
                }
